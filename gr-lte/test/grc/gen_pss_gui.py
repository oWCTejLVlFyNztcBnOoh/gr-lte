...
# Title: Gen Pss Gui
# Generated: Sun Oct 21 16:31:55 2012
##################################################
...
class gen_pss_gui(grc_wxgui.top_block_gui):
	...
	def __init__(self, N_re=128, N_cp_ts=144, freq_corr=0):
		...
		self.any_source_0_0 = pss_source_td(0,N_re,N_cp_ts,freq_corr,repeat=True) 

		##################################################
		# Connections
		##################################################
		self.connect((self.gr_throttle_0, 0), (self.wxgui_fftsink2_0, 0))
		self.connect((self.any_source_0_0, 0), (self.gr_throttle_0, 0))
...
if __name__ == '__main__':
	...
	tb = gen_pss_gui(N_re=options.N_re, N_cp_ts=options.N_cp_ts, freq_corr=options.freq_corr)
	tb.Run(True)
